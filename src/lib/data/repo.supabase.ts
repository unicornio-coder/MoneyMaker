...
const aMov = (f: Fila): Movimiento => ({ id: String(f.id), cuentaId: String(f.account_id), fecha: String(f.fecha), descripcionRaw: String(f.descripcion_raw), comercio: String(f.comercio), comercioDominio: str(f.comercio_dominio), monto: Number(f.monto), tipo: f.tipo as Movimiento['tipo'], categoriaId: String(f.categoria_id), categoriaFuente: f.categoria_fuente as Movimiento['categoriaFuente'], esMsi: !!f.es_msi, msiCuota: num(f.msi_cuota), msiTotal: num(f.msi_total), recurrenteId: str(f.recurrent_id), fuente: f.fuente as Movimiento['fuente'], hash: String(f.hash), detalle: str(f.detalle), recibo: (f.recibo as Record<string, unknown> | null) ?? null });
