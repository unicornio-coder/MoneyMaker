...
export function repoSupabaseCon(cli: () => Cli): Repo {
  return {
    ...
    async actualizarMovimiento(userId, id, c) {
      ...
      if (c.detalle !== undefined) fila.detalle = c.detalle;
      if (c.recibo !== undefined) fila.recibo = c.recibo;
      if (c.nota !== undefined) fila.nota = c.nota;
    ...
    async conciliarRecurrentes(userId, detectados) {
      ...
      for (const d of detectados) {
        ...
        let id: string;
        if (ex && (ex.origen === 'manual' || ex.canceladoAt || ex.ignorado)) id = ex.id;
        else if (ex) {
      ...
      const desactivar = existentes.filter((r) => !vistos.has(r.id) && r.origen === 'detectado' && !r.canceladoAt && !r.ignorado && r.activo).map((r) => r.id);
