...
| **3. Datos entrando solos** | Correo reenviado listo con Resend inbound (variable de entorno), sincronización automática cada hora (cron ya existe), reintentos y estado de cada fuente en Ajustes; resumen del domingo por correo | El usuario conecta una vez y todo llega solo |
...
6. **Belvo producción (opcional, después)**: contrato y llaves; el código ya está.
